...
Throughout this course, we have analyzed the computational cost of NLA algorithms by counting flops. However, as CPU processing speeds have accelerated exponentially over the last 60 years, arithmetic operations are rarley the real bottleneck. Instead, manipulating and storing the data being processed is! Most high quality scientific computing libraries for linear algebra (e.g., LAPACK) are built on highly optimized Basic Linear Algebra Subprograms (BLAS), which perform basic operations like matrix-vector operations (BLAS Level 2) and matrix-matrix operations (BLAS Level 3). These routines exploit structure and locality in the memory cache hierarchy by (among other things) blocking matrix-matrix operations. These tuned routines perform near the "peak" theoretical flop count and, consequently, counting flops turns out to be a reasonable (though crude) way to compare the performance of many core NLA routines on standard architectures. The ideal cache model provides insight into the power of blocking and also points us twoard cache-oblivious algorithms, which can be surprisingly close to optimal while being simple to code.

**Further Reading:** See the iJulia [notebook](https://github.com/mitmath/18335/blob/spring21/notes/Memory-and-Matrices.ipynb) put together by Steven G. Johnson with experiments on the performance of matrix-matrix multiplication and dot products.
