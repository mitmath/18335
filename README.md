...
### Lecture 11
During the past few weeks, we have focused on the numerical solution of linear systems, Ax = b, by _triangularizing_ the coefficient matrix A. We now turn to a second classical problem in linear algebra: the eigenvalue problem, Ax = lambda x. The eigenvalues (lambda) and eigenvectors (x) of matrices play a central role in the _diagonalization_ of a matrix. Not every matrix is diagonalized by its eigenvectors, but every **symmetric matrix is diagonalized by an orthonormal set of eigenvectors**. The eigenvectors of the symmetric nonnegative matrices A * A^T and A^T * A are the left and right singular vectors of A, respectively, while the singular values are the (non-negative) square roots of their shared eigenvalues.
