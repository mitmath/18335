...
Computing A = LU by Gaussian elimination (GE) fails entirely if a row operation involves division by zero. Similarly, dividing by a very small number can introduce large backward errors because very large numbers in L and U can introduce to catastrophic cancellation errors in the product LU. In general, the backward error can be large when ||L|| and/or ||U|| are much larger than ||A||. To control growth in the LU factors, we can use row permutations to avoid dividing by small numbers during elimination. This strategy is called partial pivoting.
...
**Further Reading:** L.N. Trefethen, Lectures 21-22. Also, experiment with the [GE Notebook](notes/Is-Gaussian-Elimination_Unstable.ipynb).
